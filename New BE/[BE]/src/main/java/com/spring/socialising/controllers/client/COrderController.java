...
public class COrderController {
    ...
    public ResponseEntity<ResponseData> uploadOrder(@RequestBody OrderDTO orderDTO){
        if(StringUtils.isEmpty(orderDTO.getReceive_name())
            ...
            return new ResponseEntity<>(ResponseData.builder()
                    .success(false)
                    .message("Information of customer is required")
                    .data(orderDTO)
                    .build(),BAD_REQUEST);
        }
        ...
        if(!StringUtils.isEmpty(orderDTO.getPromotion_code())){
            ...
            if(promotionEntity==null){
                return new ResponseEntity<>(ResponseData.builder()
                        .success(false)
                        .message("Invalid promotion code: "+orderDTO.getPromotion_code())
                        .data(orderDTO)
                        .build(),BAD_REQUEST);
            }
        }
        ...
        for (ProductEntity product: orderDTO.getProduct_list()) {
            ...
            if(productFound == null){
                return new ResponseEntity<>(ResponseData.builder()
                        .success(false)
                        .message("Invalid product with id: "+product.getId())
                        .data(orderDTO)
                        .build(),BAD_REQUEST);
            }
            else{
                if(productFound.getAmount() - product.getAmount() < 0){
                    return new ResponseEntity<>(ResponseData.builder()
                            .success(false)
                            .message("Invalid amount with id: "+product.getId())
                            .data(orderDTO)
                            .build(),BAD_REQUEST);
                }
            }
        ...
        return new ResponseEntity<>(ResponseData.builder()
                .success(true)
                .message("invoice have been created")
                .data(sellInvoiceEntity)
                .build(),OK);
    }
    ...
    public ResponseEntity<ResponseData> findAllOrderByCustomer(){
        ...
        return new ResponseEntity<>(ResponseData.builder()
                .success(true)
                .message("list order by custonmer")
                .data(orderService.findByCustomer(Long.parseLong(userDetails.getId())))
                .build(),OK);
